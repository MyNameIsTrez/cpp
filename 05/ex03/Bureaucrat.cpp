...
void Bureaucrat::signForm(AForm &form, std::ostream &ostream) const
{
	try
	{
		form.beSigned(*this);
...
void Bureaucrat::executeForm(const AForm &form, std::ostream &ostream) const
{
	try
	{
		form.execute(*this, ostream);
		ostream
			<< _name
			<< " executed "
			<< form.getName()
